/**
 * Jest configuration for the Expo / React Native app.
 *
 * `jest-expo` wires up the RN preset (Babel transform, module mocks, the
 * Hermes-like environment). We extend its `transformIgnorePatterns` so the
 * third-party ESM packages we use (react-navigation, @tanstack, expo modules)
 * are transpiled instead of being loaded raw.
 */
module.exports = {
  preset: 'jest-expo',
  setupFilesAfterEnv: ['<rootDir>/jest.setup.js'],
  transformIgnorePatterns: [
    'node_modules/(?!((jest-)?react-native|@react-native(-community)?|expo(nent)?|@expo(nent)?/.*|@expo-google-fonts/.*|react-navigation|@react-navigation/.*|@unimodules/.*|unimodules|sentry-expo|native-base|react-native-svg|@tanstack/.*))',
  ],
  collectCoverageFrom: [
    'src/**/*.{ts,tsx}',
    '!src/**/*.d.ts',
    '!src/api/mockData.ts',
  ],
  coverageReporters: ['text-summary', 'lcov'],
};
